...
const showResult2D = (title, containerId, dataArray) => {
    ...
    for (let i = 0; i < dataArray.length; i++) {
        ...
        for (let j = 0; j < dataArray[i].length; j++) {
            ...
            // Calculate the index in the dataArray based on current row and column
            let index = j;
            if (index < dataArray[i].length) {
                span.innerHTML = dataArray[i][index];
            }
            td.appendChild(span);
            tr.appendChild(td);
        }
...
function performOperation(operation) {
    ...
    if (result == -1) {
        window.alert("Matrices are of incorrect dimensions to perform operations.");
    } else {
...
function addMatrices(matrix1, matrix2){ 
    if (matrix1.length != matrix2.length || matrix1[0].length != matrix2[0].length) {return -1;}
...
const multiplyMatrices = (matrix1, matrix2) => { 
    if (matrix1.length != matrix2[0].length) {return -1;}
	let matrix3 = [];
    for (let i = 0; i < matrix1.length; i++) {
        matrix3[i] = [];
        for (let j = 0; j < matrix1[i].length; j++) { 
